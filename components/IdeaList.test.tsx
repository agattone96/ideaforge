...
describe('IdeaList Component', () => {
  beforeEach(() => {
    ...
    (projectService.deleteIdeaFromProject as jest.MockedFunction<typeof projectService.deleteIdeaFromProject>).mockImplementation(() => {});
    (projectService.updateProject as jest.MockedFunction<typeof projectService.updateProject>).mockImplementation(() => {});
    (projectService.addAttachmentsToProject as jest.MockedFunction<typeof projectService.addAttachmentsToProject>).mockReturnValue(sampleProjectWithAttachments);
    (projectService.deleteAttachmentFromProject as jest.MockedFunction<typeof projectService.deleteAttachmentFromProject>).mockReturnValue(sampleProject);
  ...
  test('renders project details and ideas', () => {
    ...
    expect(screen.getByText(`Project: ${sampleProject.name}`)).toBeInTheDocument();
  ...
  test('handles deleting an idea', async () => {
    ...
    expect(screen.getByText(`Delete Idea "${sampleIdea1.title}"?`)).toBeInTheDocument();
    ...
    expect(projectService.deleteIdeaFromProject).toHaveBeenCalledWith(sampleProject.id, sampleIdea1.id);
    expect(mockAddNotification).toHaveBeenCalledWith(`Idea "${sampleIdea1.title}" deleted.`, 'success');
  ...
  test('handles project export', async () => {
    ...
    expect(mockAddNotification).toHaveBeenCalledWith(`Project "${sampleProject.name}" export (ZIP) initiated.`, 'success');
  });
  ...
  test('handles project logo upload and removal', async () => {
    ...
    expect(projectService.updateProject).toHaveBeenCalledWith(expect.objectContaining({ logo: 'data:image/png;base64,mocklogo' }));
    expect(mockAddNotification).toHaveBeenCalledWith('Project logo updated successfully.', 'success');
    expect(mockRefreshProject).toHaveBeenCalledWith(sampleProject.id);
    ...
    expect(projectService.updateProject).toHaveBeenCalledWith(expect.objectContaining({ logo: undefined }));
    expect(mockAddNotification).toHaveBeenCalledWith('Project logo has been cleared.', 'info');
  });
  ...
  test('handles project attachment upload, download, and deletion', async () => {
    ...
    expect(projectService.addAttachmentsToProject).toHaveBeenCalledWith(
      sampleProject.id,
      expect.arrayContaining([
        expect.objectContaining({ name: 'report.txt', content: 'mock text' })
      ])
    );
    expect(mockAddNotification).toHaveBeenCalledWith('1 file(s) added to the project.', 'success');
    ...
    const projectWithAttachmentForTest = {
        ...sampleProject,
        attachments: [
            { id: 'att1', name: 'report.txt', type: 'text' as 'text', mimeType: 'text/plain', content: 'bW9jayB0ZXh0', size: 100 } // 'mock text' base64 encoded
        ]
    };
    ...
    expect(mockAddNotification).toHaveBeenCalledWith('Downloading file "report.txt"...', 'info');
    ...
    expect(window.confirm).toHaveBeenCalledWith("Are you sure you want to remove this file from the project?");
    expect(projectService.deleteAttachmentFromProject).toHaveBeenCalledWith(sampleProject.id, 'att1');
    expect(mockAddNotification).toHaveBeenCalledWith('Project file removed.', 'info');
  });
  
  test('sorts ideas based on selection', async () => {
    renderIdeaList(sampleProject);
    const sortSelect = screen.getByLabelText(/Sort ideas by:/i) as HTMLSelectElement;
  ...
  test('displays message when no ideas are present', () => {
    ...
    expect(screen.getByText(/This project has no ideas yet./i)).toBeInTheDocument();
    expect(screen.getByText(/Create your first idea by selecting 'New Idea'./i)).toBeInTheDocument();
  });

  test('displays message when no project attachments are present', () => {
    renderIdeaList({ ...sampleProject, attachments: [] });
    expect(screen.getByText(/No files have been added to this project yet./i)).toBeInTheDocument();
  });
});