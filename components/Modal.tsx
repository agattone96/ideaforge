...
const Modal: React.FC<ModalProps> = ({ 
  ...
  const handleKeyDown = useCallback((event: KeyboardEvent) => {
    ...
    if (event.key === 'Tab') {
      const focusableElementsRaw = modalRef.current?.querySelectorAll<HTMLElement>(
        'button, [href], input, select, textarea, [tabindex]:not([tabindex="-1"])'
      );
      if (!focusableElementsRaw) return;

      // Filter for visible and enabled elements to create a true focus trap
      const focusableElements = Array.from(focusableElementsRaw).filter(
