...
try {
  ...
  if (apiKey) {
    ...
    // API_KEY environment variable not found. Gemini features will be disabled.
    // Notify user via UI if needed.
  }
} catch (error) {
  // Failed to initialize GoogleGenAI. Notify user via UI if needed.
  ai = null; 
}
...
export async function withRetries<T>(asyncFn: () => Promise<T>, operationName: string = "Operation"): Promise<T> {
  ...
  while (attempts < MAX_RETRIES) {
    try {
      ...
      attempts++;
      // Retry failed. Notify user via UI if needed.
      if (attempts >= MAX_RETRIES) {
        // All attempts failed. Notify user via UI if needed.
...
export const testAiConnection = async (): Promise<{ success: boolean; data?: string; message?: string }> => {
  ...
  try {
    ...
    // AI Connection Test Failed. Notify user via UI if needed.
...
Respond with ONLY the JSON object.`;
  ...
  try {
    ...
    // Failed to parse boilerplate JSON from AI. Notify user via UI if needed.
...
function getRGBComponents(hexColor: string): string {
  ...
  if (color.length !== 6) {
    // Invalid hex color for RGB conversion. Falling back to black. Notify user via UI if needed.
    return '0, 0, 0';
  }
...
export const importProjectsFromJson = (jsonString: string): { success: boolean, message: string, projectsImported?: number } => {
  try {
    ...
    const errorMessage = error instanceof Error ? error.message : String(error);
    // Import failed. Notify user via UI if needed.
