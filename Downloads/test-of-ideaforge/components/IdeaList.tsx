...
const IdeaList: React.FC<IdeaListProps> = ({ project, onEditIdea, onCreateNewIdea, onBackToProjects, addNotification, onUpdateProject }) => {
  ...
  const handleExportProject = async () => {
    try {
      ...
      addNotification(`Failed to export constellation: ${error instanceof Error ? error.message : String(error)}`, 'error');
      // Error already surfaced to user.
    }
  };
  ...
  const handleLogoUpload = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      addNotification('Sigil upload failed. Try a different cosmic image.', 'error');
      // Error already surfaced to user.
    }
