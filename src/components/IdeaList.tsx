import React, { useState, useRef, ChangeEvent, useCallback, lazy, Suspense } from 'react';
...
// Skeleton Idea List Component
import type { Idea } from '../types';
// Lazy-load Modal to reduce initial bundle size
const Modal = lazy(() => import('./Modal')); // Import Zine Exporter
// Lazy-load BlueprintCard to reduce initial bundle size
const BlueprintCard = lazy(() => import('./BlueprintCard'));
    ...
    >
      ...
      <div className="mb-8">
        ...
        {sortedIdeas.length > 0 ? (
          <div className="h-[70vh]">
            <AutoSizer>
              {({ height, width }) => {
                ...
                const rowCount = Math.ceil(sortedIdeas.length / columnCount);
                const columnWidth = width / columnCount;
                const rowHeight = 350; // adjust to match card height
                return (
                  <Grid
                  ...
                  >
                    {({ columnIndex, rowIndex, style }) => {
                      ...
                      if (index >= sortedIdeas.length) return null;
                      const idea = sortedIdeas[index];
                      return (
                        <div style={style} className={BLUEPRINT_CARD_ITEM_CLASS}>
                          <BlueprintCard
                            item={idea}
                            type="idea"
          ...
          >
            <DocumentTextIcon className="w-16 h-16 sm:w-20 sm:w-20 mx-auto text-theme-accent-primary/50 mb-4 opacity-70" />
